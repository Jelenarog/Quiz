...
var createQuestion = function () {
  ...
  for (let i = 0; i < questionsAndAnswers[counter].answer.length; i++) {
    //for each question show answer choices
...
var questionOption = function (event) {
  ...
  if (counter == questionsAndAnswers.length - 1) {
    ...
    //for each time user selects one of li elements(answer choices display if answer is correct or incorrect)
    ...
    if (counter < questionsAndAnswers.length) {
      //if there is more  questions available call the function to create question again
...
var endQuiz = function () {
  //whe game is done show end screen where user can see his final score
